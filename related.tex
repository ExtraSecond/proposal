...
\subsection{\todo{Operating Systems with C\#}}

\todo{C\# open source managed operating system (COSMOS) is written in C\#. They first translate C\# into Common Intermediate Language and then use IL2CPU (IL to CPU) to translate CIL to machine code for CPU. IL2CPU is an AOT compiler that is written in a Common Intermediate Language compliant language (C\#). Now they only achieve a High Level Assembler (HLA) for X86/X64 assembly. In the future, other processors are planned.}
